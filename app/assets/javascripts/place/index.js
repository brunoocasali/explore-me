...
*
* Invokes rails /places/create by ajax
* Add the form to google maps infowindow
*/
function placesNew(geocode_information) {

    // Create an new marker
    placesNewMarker = new google.maps.Marker({
    ...
    $.ajax({
        url: '/places/new?' + jQuery.param({place:geocode_information}) + '#chunked=true',
        type: 'GET',
        async: false,
        success: function(html) {
            ...
            google.maps.event.addListener(placesNewMarker, "click", function() {
                createFormOpen();
            });
        }
    });
...
function geocodePoint(latlng, callback) {
    ...
    geocoder.geocode({'latLng': latlng }, function(responses) {
        ...
        if (responses && responses.length > 0) {
              ...
              name = 'Place at ' + responses[0].formatted_address;
